...
total_us_cases <- pull(covid_data, sum(cases))
...
state_highest_ratio <- pull(covid_data, state, max(deaths/cases))
...
state_lowest_cases <- pull(covid_data, state, min(cases))
...
national$new_cases <- lag(national$cases, n = 1, default = 0)
...
highest_cases_in_each_state = df.groupby(['state_name'])['cases'].max().reset_index()
highest_cases_in_each_state.columns = ['state_name', 'highest_cases']
...
lowest_deaths_in_each_state = df.groupby(['state_name'])['deaths'].min().reset_index()
lowest_deaths_in_each_state.columns = ['state_name', 'lowest_deaths']
...
## Calculate the average number of cases per day for each county
#

#  Write code (at least 2-3 lines) that will answer your question
my_answer <- avg_cases_per_day <- counties %>%
